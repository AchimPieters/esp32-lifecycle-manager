...
### Default signing behavior (noob-proof)

This repository ships with a **default development OTA keypair** in `keys/` so a new user can clone, build, sign, and publish immediately without creating keys first.

- `keys/ota_signing_private.pem` is used automatically by `generate_sig.sh`.
- `keys/ota_signing_public.pem` matches the public key embedded in LCM verification logic.

This makes the starter flow work out-of-the-box. For production, switch to your own keypair (see **Advanced custom signing keys** below).
...
### Fast path (default key, no setup)

```bash
./generate_sig.sh build/main.bin
```

This uses `keys/ota_signing_private.pem` automatically.

### Advanced path (custom private key)

```bash
./generate_sig.sh build/main.bin /path/to/ota_signing_private.pem
```

If you use a custom private key, you must also compile LCM with the matching
public key (replace `OTA_PUBLIC_KEY_PEM` in `main/github_update.c`).
