...
### Standard flow (out-of-the-box demo keys)

After cloning this repository, you can sign immediately with the included demo key:

```bash
./generate_sig.sh build/main.bin
```

You can also point directly to any firmware image path, for example:

```bash
./generate_sig.sh examples/led/build/main.bin
```

### Advanced flow (custom keys)

Use your own signing key by passing it as the second argument:

```bash
./generate_sig.sh build/main.bin /path/to/ota_signing_private.pem
```

To make LCM verify your custom key, set `LCM_USE_CUSTOM_OTA_PUBLIC_KEY=y` and
provide `LCM_CUSTOM_OTA_PUBLIC_KEY_PEM` (single-line PEM with `\n` escapes)
